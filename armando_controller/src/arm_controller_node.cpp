  ...
  private:
    void topic_callback(const sensor_msgs::msg::JointState & msg) const {
      if (msg.position.size() >= 4) {
        ...
        for (int i = 0; i < 4; i++) {
          RCLCPP_INFO(this->get_logger(), "Joint Position %d: '%6f'", i, (double)msg.position[i]);
        }
      }
    }
...
class PositionControllerPublisher : public rclcpp::Node
{
  public:
    ...
    : Node("position_controller_publisher"), count_(0)
    {
      ...
      5000ms, std::bind(&PositionControllerPublisher::timer_callback, this));
    }
  ...
  private:
    void timer_callback()
    {
      ...
      auto commands_msg = std_msgs::msg::Float64MultiArray();
      commands_msg.data = current_command;
      RCLCPP_INFO(this->get_logger(), "Publishing command set %zu:", count_);
...
class TrajectoryControllerPublisher : public rclcpp::Node
{
  public:
    ...
    : Node("trajectory_controller_publisher"), count_(0) {
      ...
      timer_ = this->create_wall_timer(
      5000ms, std::bind(&TrajectoryControllerPublisher::timer_callback, this));
    }
...
int main(int argc, char * argv[])
{
  ...
  bool controller_type = param_node->declare_parameter<bool>("controller_type", true);
